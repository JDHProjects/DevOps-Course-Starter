...
def create_app():
  ...
  def completed_item(id):
    board_items.change_item_list(id, os.getenv('TRELLO_COMPLETED_LIST'))
    return redirect("/")

  @app.route('/in-progress/<id>')
  def in_progress_item(id):
    board_items.change_item_list(id, os.getenv('TRELLO_IN_PROGRESS_LIST'))
    return redirect("/")

  @app.route('/not-started/<id>')
  def not_started_item(id):
    board_items.change_item_list(id, os.getenv('TRELLO_NOT_STARTED_LIST'))
    return redirect("/")
